...
export default function PerfilEmpresa() {
  ...
  useEffect(() => {
    profileService.get('company').then((data) => {
      const values = {
        name: data.name || '',
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await profileService.update('company', { name, description });
      setSaved({ ...form });
      setIsEditing(false);
      await refreshUser();
    } catch (err) {
  ...
  return (
    <div className="perfil-form">
      <div className="perfil-form-header">
        <div>
          <h3 className="perfil-title">Información de la empresa</h3>
          <p className="perfil-subtitle">Administra los datos públicos de tu empresa</p>
        </div>
        <button
          type="button"
      ...
      <div className="perfil-grid perfil-grid-1">
        <Campo label="Nombre de la empresa" name="name" value={isEditing ? form.name : saved.name} onChange={handleChange} disabled={!isEditing} />
